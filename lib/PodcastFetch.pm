...
sub rewrite_tags {
  ...
  # stupid workaround for error messages generated by MP3::Tag that can't be turned off.
  open OLDOUT,     ">&", \*STDOUT or die "Can't dup STDOUT: $!";
  open OLDERR,     ">&", \*STDERR or die "Can't dup STDERR: $!";
  open STDOUT, ">","/dev/null";
  open STDERR, ">","/dev/null";
  my $tag = MP3::Tag->new($filename) or return;
  $tag->update_tags({title=>$title,genre=>'Podcast'});
  open STDOUT, ">&",\*OLDOUT;
  open STDERR, ">&",\*OLDERR;
